...
void Generator::PrintC(const std::list<std::unique_ptr<Node>>& nodes) {
  std::cout
    << "#include <stdio.h>\n"
    << "#include <stdlib.h>\n\n"
    << "int main(void) {\n"
...
void Generator::PrintX64(const std::list<std::unique_ptr<Node>>& nodes) {
  int label = 4;
...
void Generator::GenerateX64(const std::unique_ptr<Node>& node, int& label) {
  switch (node->kind) {
  case ND_INC_PTR:
