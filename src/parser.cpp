...
Parser::Parser(): token_iter_{}, token_end_{}, nodes_{}, depth_{0} {}

void Parser::Initialize(const std::list<std::unique_ptr<Token>>& tokens) {
  token_iter_ = tokens.begin();
  token_end_ = tokens.end();
  nodes_ = std::list<std::unique_ptr<Node>>{};
  depth_ = 0;
}
